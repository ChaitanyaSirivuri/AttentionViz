...
def handle_attentions_i2t(state, highlighted_text, layer_idx=32, token_idx=0):
    ...
    if highlighted_text is not None:
        ...
        token_idx_list = []
        # highlighted_text is expected to be a list of dicts or similar structure from Streamlit if adapted
        # For now assuming it's passed as a list of selected tokens or similar
        # If it's just a list of strings:
        if isinstance(highlighted_text, list):
             for item in highlighted_text:
                # Adaptation for Streamlit: item might be just the string or a dict
        ...
        if not token_idx_list:
            ...
            logger.error("Selected text not found in generated output")
            return None, None, [], None
        
        generated_text = []
        # Reconstruct generated text format if needed, or just return list of strings
        # adapting to return what Streamlit needs
    else:
        token_idx_list = [0]

    if not os.path.exists(fn_attention):
        # gr.Error('Attention file not found. Please re-run query.')
        logger.error('Attention file not found. Please re-run query.')
        return None, None, [], None
    else:
        ...
        if len(attentions) == len(state.output_ids_decoded):
            logger.error('Mismatch between lengths of attentions and output tokens')
        ...
        ax.set_title(f"Mean Attention for layer {layer_idx+1}")
...
def boxes_click_handler(image, box_grid, x, y):
    # Modified to take x, y directly instead of event
